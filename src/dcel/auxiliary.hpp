...
bool isClockwise(const std::vector<std::shared_ptr<Vertex<GeographicPoint>>>& vertices) {
...
std::vector<std::shared_ptr<HalfEdge<GeographicPoint>>> genGeographicPoints(const std::shared_ptr<DCEL<GeographicPoint>>& dcel,
