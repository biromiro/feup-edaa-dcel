...
{
    bool operator()(const std::shared_ptr<Face<T>> lhs, const std::shared_ptr<Face<T>> rhs) const
    {
        return *lhs < *rhs;
    }
};
...
public:
    ...
    std::set<std::shared_ptr<Face<T>>, FaceComparator<T>> getFaces() { return this->faces; };
    std::set<std::shared_ptr<HalfEdge<T>>> getEdges() { return this->edges; };
    std::set<std::shared_ptr<Vertex<T>>, VertexComparator<T>> getVertices() { return this->vertices; };
    ...
    std::shared_ptr<Vertex<T>> getVertex(const T& val) {
        ...
        return value == vertices.end() ? std::shared_ptr<Vertex<T>>(nullptr) : (*value);
    }

    std::shared_ptr<HalfEdge<T>> getHalfEdge(const std::shared_ptr<Vertex<T>>& orig, const std::shared_ptr<Vertex<T>>& dest) {
        auto edge = std::find_if(
                edges.begin(),
                edges.end(),
                [&orig, &dest](const std::shared_ptr<HalfEdge<T>>& hedge) {
                    const std::shared_ptr<Vertex<T>>& originVertex = hedge->getOrigin();
                    const std::shared_ptr<Vertex<T>>& destVertex = (hedge->getTwin())->getOrigin();
                    return (originVertex.get() == orig.get()) && (destVertex.get() == dest.get());
                });

        return edge == edges.end() ? std::shared_ptr<HalfEdge<T>>(nullptr) : (*edge);
    }

    std::shared_ptr<Face<T>> getFace(const nlohmann::json& property) {
        auto value = faces.find(std::make_shared<Face<T>>(property));

        return value == vertices.end() ? std::shared_ptr<Face<T>>(nullptr) : (*value);
    }
