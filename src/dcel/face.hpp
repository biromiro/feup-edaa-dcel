...
class Face {
 public:
    ...
    Face(nlohmann::json property) : outer(), inner(), property(std::move(property)) {}
    Face(Face<T> &&) = default;
    Face(const Face<T> &) = default;
    Face<T> &operator=(Face<T> &&) = default;
    Face<T> &operator=(const Face<T> &) = default;

    [[nodiscard]] const nlohmann::json &getProperty() const {
        return property;
    }

    const std::shared_ptr<HalfEdge<T>> &getOuter() const {
        return outer;
    }
